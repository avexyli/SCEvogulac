...
function updateRatings() {
  ...
  const SCORE_A = headers.indexOf('ScoreA');
  const SCORE_B = headers.indexOf('ScoreB');
  const DATE = headers.indexOf('Date');

  const matches = data.map(row => {
    const a = row[PLAYER_A]?.toString().trim();
    const b = row[PLAYER_B]?.toString().trim();
    const scoreA = parseInt(row[SCORE_A]);
    const scoreB = parseInt(row[SCORE_B]);
    const total = scoreA + scoreB;
    return {
      a,
      b,
      result: total > 0 ? scoreA / total : null,
...
let playerInfoMap = new Map();
const playerRowCount = infoSheet.getLastRow() - 1;
if (playerRowCount > 0) {
  playerInfoMap = new Map(
    infoSheet.getRange(2, 1, playerRowCount, 2).getValues().map(([name, race]) => [name?.trim(), race])
  );
}
  );
...
let existingInfo = [];
const existingCount = infoSheet.getLastRow() - 1;
if (existingCount > 0) {
  existingInfo = infoSheet.getRange(2, 1, existingCount, 1).getValues().flat().map(n => n.trim());
}
...
let allInfo = [];
const rowCount = infoSheet.getLastRow() - 1;
if (rowCount > 0) {
  allInfo = infoSheet.getRange(2, 1, rowCount, 2).getValues();
}
  const filtered = allInfo.filter(([name]) => toKeep.includes(name?.trim()));
  safeClearPlayerInfo(infoSheet);
...
let ratingsData = [];
const ratingsRowCount = ratingSheet.getLastRow() - 1;
if (ratingsRowCount > 0) {
  ratingsData = ratingSheet.getRange(2, 1, ratingsRowCount, 2).getValues();
}
let playerInfoData = [];
const playerInfoRowCount = infoSheet.getLastRow() - 1;
if (playerInfoRowCount > 0) {
  playerInfoData = infoSheet.getRange(2, 1, playerInfoRowCount, 2).getValues();
}
...
function safeClearPlayerInfo(infoSheet) {
  const oldRowCount = infoSheet.getLastRow() - 1;
  if (oldRowCount > 0) {
    infoSheet.getRange(2, 1, oldRowCount, 2).clearContent();
  }
}
