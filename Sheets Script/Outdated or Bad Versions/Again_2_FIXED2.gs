...
function updateRatings() {
  ...
  // Load last processed date from a reserved cell, or default to the epoch start
  const metaSheet = sheet.getSheetByName('Meta') || sheet.insertSheet('Meta');
  const lastProcessedDateValue = metaSheet.getRange('A1').getValue();
  const lastProcessedDate = (lastProcessedDateValue instanceof Date && !isNaN(lastProcessedDateValue)) 
    ? lastProcessedDateValue 
    : new Date(0);
  ...
  const lastProcessedDateCell = sheet.getRange('Settings!A1');
  const lastProcessedDateValue = lastProcessedDateCell.getValue();
  const lastProcessedDate = lastProcessedDateValue ? new Date(lastProcessedDateValue) : new Date(0);

  const newMatches = data
    .map(row => {
      const a = row[PLAYER_A]?.toString().trim();
      const b = row[PLAYER_B]?.toString().trim();
      const scoreA = parseFloat(row[RESULT]);
      const date = new Date(row[DATE]);
      return {
        a,
        b,
        result: isNaN(scoreA) ? null : scoreA,
        date: isNaN(date.getTime()) ? null : date
      };
    })
    .filter(row => row.a && row.b && row.result !== null && row.date && row.date > lastProcessedDate)
    .sort((x, y) => x.date - y.date);

  if (newMatches.length === 0) {
    Logger.log("No new matches found since last update.");
    return;
  }

  sheet.getRange('Settings!A1').setValue(newMatches[newMatches.length - 1].date);
