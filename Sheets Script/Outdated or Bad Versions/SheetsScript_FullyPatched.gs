...
function updateRatings() {
  ...
  // Efficient Warnings scan using Settings!A1
  const settingsSheet = sheet.getSheetByName('Settings') || sheet.insertSheet('Settings');
  const lastScanCell = settingsSheet.getRange('A1');
  const lastScanValue = lastScanCell.getValue();
  const lastScanDate = lastScanValue instanceof Date ? new Date(lastScanValue) : new Date(0);

  const newlyParsedMatches = matches.filter(m => m.date > lastScanDate);

  const newNames = Array.from(new Set(newlyParsedMatches.flatMap(m => [m.a, m.b])));
  const warnings = [];
  ...
  lastScanCell.setValue(new Date());
