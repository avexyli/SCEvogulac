...
let warnings = [];  // moved here to ensure early declaration
  ...
  const lastProcessedCell = settingsSheet.getRange('B1');
  const lastProcessedDateValue = lastProcessedCell.getValue();
  const lastProcessedDate = lastProcessedDateValue instanceof Date ? lastProcessedDateValue : new Date('2000-01-01');

  const newMatches = matches.filter(m => m.date > lastProcessedDate);

  const warnings = [];
  const newNames = Array.from(new Set(newMatches.flatMap(m => [m.a, m.b])));

  for (let i = 0; i < newNames.length; i++) {
    for (let j = i + 1; j < newNames.length; j++) {
      const sim = stringSimilarity(newNames[i], newNames[j]);
      if (sim > 0.85 && sim < 1) {
        warnings.push(`!!! Possible duplicate: "${newNames[i]}" vs "${newNames[j]}" (similarity: ${Math.round(sim * 100)}%)`);
      }
    }
  }
  ...
  if (warnings.length > 0) {
    ...
    warningSheet.getRange(startRow, 1).setValue(':D No possible duplicates found.');
  }

  // Update last processed date
  const latestDate = newMatches.reduce((latest, m) => m.date > latest ? m.date : latest, lastProcessedDate);
  lastProcessedCell.setValue(latestDate);
