...
function updateRatings() {
  ...
  const lastProcessedDateValue = settingsSheet?.getRange('B1').getValue();
  const lastProcessedDate = lastProcessedDateValue instanceof Date && !isNaN(lastProcessedDateValue.getTime())
    ? new Date(lastProcessedDateValue)
    : new Date(0); // fallback to epoch if not valid
...
let warnings = [];  // moved here to ensure early declaration
  ...
  if (newMatches.length > 0 && settingsSheet) {
    const latestDate = newMatches[newMatches.length - 1].date;
