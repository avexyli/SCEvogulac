...
function updateRatings() {
  ...
  const lastProcessedValue = lastProcessedCell.getValue();
  const lastProcessedDate = lastProcessedValue instanceof Date ? lastProcessedValue : new Date(0);
...
const matches = data.map(row => {
  ...
  }).filter(row => row.a && row.b && row.result !== null && row.date > lastProcessedDate)(row => row.a && row.b && row.result !== null && row.date)
...
matches.forEach((m, i) => {
  ...
  const newestDate = matches.reduce((latest, m) => m.date > latest ? m.date : latest, lastProcessedDate);
